...
describe("Voice Bios Routes", () => {
			...
			mockSupabase.from.mockReturnValueOnce({
				insert: jest.fn().mockReturnValue({
					select: jest.fn().mockReturnValue({
						single: jest
							.fn()
							.mockResolvedValue({ data: createdBio, error: null }),
					}),
				}),
			});
			...
			mockSupabase.from.mockReturnValueOnce({
				select: jest.fn().mockReturnValue({
					eq: jest.fn().mockReturnValue({
						single: jest
							.fn()
							.mockResolvedValue({ data: existingBio, error: null }),
					}),
				}),
			});
				...
				update: jest.fn().mockReturnValue({
					eq: jest.fn().mockReturnValue({
						select: jest.fn().mockReturnValue({
							single: jest
								.fn()
								.mockResolvedValue({ data: updatedBio, error: null }),
						}),
					}),
				}),
			});
	...
	describe("GET /:userId", () => {
			...
			mockSupabase.from.mockReturnValue({
				select: jest.fn().mockReturnValue({
					eq: jest.fn().mockReturnValue({
						single: jest
							.fn()
							.mockResolvedValue({ data: voiceBio, error: null }),
					}),
				}),
			});
		...
		it("should return 404 when user has no voice bio", async () => {
			...
			const response = await request(app).get("/user-456").expect(404);
	...
	describe("PUT /:id", () => {
		...
		it("should reject update by non-owner", async () => {
			...
			expect(response.body).toHaveProperty(
				"message",
				"Not authorized to update this voice bio"
			);
		});
	...
	describe("DELETE /:id", () => {
		it("should delete voice bio by owner", async () => {
			...
			expect(response.body).toHaveProperty(
				"message",
				"Voice bio deleted successfully"
			);
		});
		...
		it("should reject delete by non-owner", async () => {
			...
			expect(response.body).toHaveProperty(
				"message",
				"Not authorized to delete this voice bio"
			);
		});
	...
	describe("Error Handling", () => {
		it("should handle database errors gracefully", async () => {
			...
			const response = await request(app).get("/user-456").expect(500);
		...
		it("should limit transcript length", async () => {
			...
			expect(response.body.message).toContain(
				"Transcript must be 1000 characters or less"
			);
		});
	});
});
