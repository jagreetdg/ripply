...
const supabaseKey = process.env.SUPABASE_KEY || 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImducGh2Zmxha215aXNpbXdueWFpIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NDQ3Njc2NzYsImV4cCI6MjA2MDM0MzY3Nn0.gtQr1Ta8P5yWjg5j7yfA0UYXzmmTjlhZgN-2Dr2OJgM';
...
async function addMockData() {
  ...
  try {
    ...
    const user1Notes = voiceNotes.filter(note => note.user_id === userIdMap.user1);
    const prometheusNotes = voiceNotes.filter(note => note.user_id === userIdMap.prometheus);
    ...
    if (commentsToAdd.length > 0) {
      ...
      if (insertCommentError) {
        ...
        newComments.forEach(comment => {
          const noteTitle = voiceNotes.find(note => note.id === comment.voice_note_id)?.title;
          const commenterUsername = Object.keys(userIdMap).find(key => userIdMap[key] === comment.user_id);
          console.log(`- Comment on "${noteTitle}": "${comment.content}" by ${commenterUsername}`);
        });
      }
    } else {
