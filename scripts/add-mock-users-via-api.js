...
async function addMockUsers() {
	try {
		...
		try {
			...
			console.error(
				`Backend API is not available. Make sure the server is running on ${API_URL.replace(
					"/api",
					""
				)}`
			);
		...
		for (const user of mockUsers) {
			try {
				...
				const existingUsers = await apiRequest(
					`${ENDPOINTS.USERS}/username/${user.username}`
				);

				if (existingUsers && existingUsers.id) {
					console.log(
						`User '${user.username}' already exists with ID: ${existingUsers.id}`
					);
					continue;
				}
...
async function addVoiceNotesForUser(userId, username) {
	...
	for (const voiceNote of voiceNotes) {
		try {
			...
			console.log(
				`Added voice note '${voiceNote.title}' for user '${username}'`
			);
			...
			console.error(
				`Error adding voice note '${voiceNote.title}' for user '${username}':`,
				error.message
			);
		}
	}
	...
	try {
		...
		for (const comment of comments) {
			...
			try {
				...
				console.error(
					`Error adding comment to voice note ${voiceNoteId}:`,
					error.message
				);
			}
		}
