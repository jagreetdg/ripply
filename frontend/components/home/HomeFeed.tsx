...
import { StyleSheet, View, Text, ActivityIndicator, TouchableOpacity, RefreshControl, ScrollView } from "react-native";
import { FeedItem } from "./FeedItem";
import { getVoiceNotes, getPersonalizedFeed } from "../../services/api/voiceNoteService";
...
export function HomeFeed() {
  ...
  const [feedType, setFeedType] = useState<'personalized' | 'all'>('personalized');
  ...
  const fetchFeedData = async (feedTypeToFetch = feedType) => {
    try {
      ...
      if (user && feedTypeToFetch === 'personalized') {
        ...
        voiceNotesData = await getVoiceNotes();
  ...
  return (
    <View style={styles.container}>
      <View style={styles.feedHeader}>
        {user ? (
          <TouchableOpacity onPress={toggleFeedType} style={styles.feedTypeToggle}>
            <Text style={styles.feedTitle}>{feedType === 'personalized' ? 'Following' : 'For You'}</Text>
            <Feather name="chevron-down" size={16} color="#6B2FBC" style={styles.toggleIcon} />
          </TouchableOpacity>
        ) : (
          ...
          <View style={styles.emptyContainer}>
            {user && feedType === 'personalized' ? (
              <>
                ...
                >
                  <Text style={styles.switchFeedButtonText}>View global feed</Text>
                </TouchableOpacity>
              </>
            ) : (
              <>
